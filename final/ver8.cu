...
void process_image_gpu(unsigned char* h_input, unsigned char* h_output, 
    ...
    for (int i = 0; i < h; ++i) {
        ...
        bit_reversal_reorder_kernel<<<numBlocks, threadsPerBlock>>>(row, row, d_indices, w);
    ...
    transpose_kernel<<<transposeGrid1, blockDim>>>(d_fft_data, d_transposed, h, w);
    ...
    for (int i = 0; i < w; ++i) {
        cuDoubleComplex* row = d_transposed + i * h;
        numBlocks = (h + threadsPerBlock - 1) / threadsPerBlock;
        
        bit_reversal_reorder_kernel<<<numBlocks, threadsPerBlock>>>(row, row, d_indices, h);
        
        for (int stage = 1; stage <= (int)log2(h); ++stage) {
            fft_kernel_row<<<numBlocks, threadsPerBlock>>>(d_transposed, h, stage, i, h);
        }
    }
    
    dim3 transposeGrid2(
        (h + blockDim.x - 1) / blockDim.x,
        (w + blockDim.y - 1) / blockDim.y
    );
    // Transpose back
    transpose_kernel<<<transposeGrid2, blockDim>>>(d_transposed, d_fft_data, w, h);
