...
__global__ void init_fft_data(unsigned char* gray, thrust::complex<double>* fft_data, 
    ...
    if (idx < w && idy < h) {
        if (idx < width && idy < height) {
            fft_data[idy * w + idx] = thrust::complex<double>((double)gray[idy * width + idx], 0);
        } else {
            fft_data[idy * w + idx] = thrust::complex<double>(0, 0);
        }
    }
}
...
void process_image_gpu(unsigned char* h_input, unsigned char* h_output, 
    ...
    // Step 1: Allocate all required GPU memory upfront
    unsigned char *d_input, *d_gray;
    thrust::complex<double> *d_fft_data;
    unsigned char *d_output;
    double *d_max_val;
    int *d_indices;
    ...
    // Step 2: Copy input data to GPU (only once)
    cudaMemcpy(d_input, h_input, input_size, cudaMemcpyHostToDevice);
    
    // Step 3: Convert to grayscale on GPU
    dim3 blockDim(32, 32);
    dim3 gridDim(
        (width + blockDim.x - 1) / blockDim.x,
        (height + blockDim.y - 1) / blockDim.y
    ...
    // Step 4: Initialize FFT data on GPU
    cudaMemset(d_max_val, 0, sizeof(double));
    
    // Convert grayscale to complex numbers directly on GPU
    init_fft_data<<<gridDim, blockDim>>>(d_gray, d_fft_data, height, width, w, h);
    
    // Step 5: Perform FFT rows
    int threadsPerBlock = 256;
    int numBlocks;
    
    // Generate and copy bit reversal indices for rows
    int* h_indices = (int*)malloc(max(w, h) * sizeof(int));
    generate_bit_reversal_indices(h_indices, w);
    cudaMemcpy(d_indices, h_indices, w * sizeof(int), cudaMemcpyHostToDevice);
    ...
    // Process rows
    for (int i = 0; i < h; ++i) {
        thrust::complex<double>* row = d_fft_data + i * w;
    ...
    // Step 7: Center frequency
    center_frequency_kernel<<<gridDim, blockDim>>>(d_fft_data, h, w);
    
    // Step 8: Compute max value and normalize
    ...
    // Step 9: Copy final result back to CPU (only once)
